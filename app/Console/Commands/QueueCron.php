...
class QueueCron extends Command
{
    ...
    public function handle()
    {
        ...
        foreach($companies as $com)
        {
            ...
            if($static)
            {
                $count=$static['messages_statistics']['queue'];
                if($count<=100)
                {
                    $page=1;
                }else
                ...
                for($i=1;$i<=$page;$i++)
                {
                    ...
                    curl_setopt_array($curl, array(
                      CURLOPT_URL => "https://api.ultramsg.com/".$com->c_instance_id."/messages?token=".$com->c_token."&page=".$i."&limit=100&status=queue",
                        ...
                        foreach($response['messages'] as $mess)
                        {
                            ...
                            if($message)
                            {
                                ...
                                if(!$sendstatus)
                                {
                                    ...
                                    Messages::where('whatsapp_message_id',$mess['id'])->update(['send_status_id'=>5]);
                                }
                            }
                        }
                    }
